...
const opportunities = [
  { symbol: 'AMD', name: 'Advanced Micro Devices', price: '$164.20', change: '+3.12%', signal: 'Buy', confidence: 88, rationale: 'Volume divergence on the 4H chart aligning with heavy call buying in the $170 strike. Supply zone broken.', tags: ['Vol Breakout', 'Options Flow'], color: 'green' },
  { symbol: 'CRWD', name: 'CrowdStrike Holdings', price: '$285.50', change: '-1.45%', signal: 'Short', confidence: 76, rationale: 'Double top formation validated by bearish MACD crossover. Insider selling cluster detected over last 7 days.', tags: ['Tech Weakness', 'Bearish Pattern'], color: 'red' },
  { symbol: 'NVDA', name: 'NVIDIA Corporation', price: '$892.40', change: '+4.28%', signal: 'Strong Buy', confidence: 95, rationale: 'Institutional accumulation detected across 3 consecutive sessions. RSI breakout above 70 with sustained volume confirming momentum.', tags: ['Breakout', 'Institutional Activity', 'Volume Spike'], color: 'green' },
];
            ...
            <div className="flex flex-col lg:flex-row items-stretch min-h-[520px]">
              ...
              <div className="relative z-10 flex-1 p-10 lg:p-14 xl:p-16 flex flex-col justify-center space-y-10 hero-stagger">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-5 pt-2 hero-stagger-item">
                  ...
                  <a href="#features" className="w-full sm:w-auto text-gray-400 hover:text-white font-medium px-6 py-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2">
          ...
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="relative">
              ...
              <div className="hidden md:block absolute top-12 left-[12.5%] right-[12.5%] h-px">
                ...
                {/* Progress fill that moves with active step */}
                <div
                  className="absolute top-0 left-0 h-full bg-gradient-to-r from-gold/40 to-gold/60 transition-all duration-700 ease-out"
                  style={{ width: `${((activeStep + 1) / 4) * 100}%` }}
                />
                {/* Glowing dot at the progress tip */}
                <div
                  className="absolute top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-gold shadow-[0_0_8px_rgba(212,175,55,0.6)] transition-all duration-700 ease-out"
                  style={{ left: `${((activeStep + 1) / 4) * 100}%` }}
                />
              </div>
                ...
                {steps.map((s, idx) => {
                  ...
                  return (
                    <div key={s.num} className="flex flex-col items-start relative group">
                      <div className={`w-24 h-24 rounded-2xl flex items-center justify-center mb-6 relative border transition-all duration-500 ${
                        isActive
                          ? 'border-gold/40 bg-[#131825] shadow-[0_0_30px_rgba(212,175,55,0.15)]'
                          : isPast
                            ? 'border-gold/15 bg-[#0f1318]'
                            : 'border-white/[0.08] bg-[#0d121c]'
                      }`}>
                        {/* Step number badge */}
                        <div className={`absolute -top-3 -left-3 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold z-10 transition-all duration-500 ${
                          isActive
                            ? 'bg-gold border-gold text-[#0A0A0A]'
                            : isPast
                              ? 'bg-gold/20 border-gold/30 text-gold'
                              : 'bg-base border border-white/10 text-gold'
                        }`}>{s.num}</div>
                        ...
                        <svg width="32" height="32" viewBox="0 0 24 24" fill="none"
                          stroke={isActive ? '#D4AF37' : isPast ? '#D4AF37' : 'currentColor'}
                          strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
                          className={`transition-all duration-500 ${isActive ? 'scale-110' : isPast ? 'opacity-70' : 'text-gray-400'}`}
                        >
                      ...
                      <h4 className={`text-lg font-semibold mb-2 transition-colors duration-500 ${isActive ? 'text-gold' : 'text-white'} group-hover:text-gold`}>{s.title}</h4>
